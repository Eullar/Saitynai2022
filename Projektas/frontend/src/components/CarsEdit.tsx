...
function CarsEdit() {
    ...
    if (!state.isInitialized) {
        backend.get<Car>(`api/RentOffices/${locationParams['id']}/Cars/${locationParams['carId']}`)
            .then(response => {
                updateState(state => {
                    const car = response.data;
                    state.manufacturer = car.manufacturer;
                    state.model = car.model;
                    state.drivetrain = car.drivetrainType;
                    state.tyre = car.tyreCompound;
                    state.transmission = car.transmissionType;
                })
            })
            .catch();
    ...
    let onSave = () => {
        update(() => {
            ...
            backend.put(`api/RentOffices/${locationParams['id']}/Cars/${locationParams['carId']}`, entity)
                .then(resp => {
